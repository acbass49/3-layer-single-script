...
#not sure where to put this fixture data, so created a new test `test_database_data` for it.
@pytest.fixture
def data_for_test():
    return [('Sammy', 'shark', 1), ('Jamie', 'cuttlefish', 7)]
...
def test_database_data(data_for_test):
    db_fresh_start()
    db = Singleton()
    db.get_cursor()
    assert data_for_test == db.sql("SELECT * FROM fish;")
